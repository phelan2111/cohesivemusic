...
function Me(props: IAlbumOfMeItemProps) {
...
function Normal(props: IAlbumOfMeItemProps) {
	const isActive = props?.id === props.item.playlistId;
	return (
		<div
			aria-hidden
			...
			<div className='size-20 min-w-20 rounded-md'>
				<img src={props.item.image} alt={props.item.namePlaylist} className='rounded-md' />
			</div>
			<div className='flex flex-col gap-1'>
				<p>{props.item.namePlaylist}</p>
				<div className='flex items-center gap-2 text-xs'>
					<p className='flex items-center text-white/70'>
						{Localize('PLAYLIST')} <LuDot /> {props.item.descriptionPlaylist}
					</p>
				</div>
			</div>
		</div>
